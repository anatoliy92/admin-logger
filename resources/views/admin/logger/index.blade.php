  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div class="collapse" id="collapseFilter">
        <div class="card">
          <div class="card-body">
            <form id="filter-form" action="{{ route('adminlogger::index') }}" method="get">
              ...
              <div class="row">
                ...
                <div class="col-12 col-sm-12 col-md-6 col-lg-4 col-xl-2">
                  <div class="form-group">
                    {{ Form::label(null, 'Before date:') }}
                    {{ Form::select('model', config('adminlogger.modelsNames'), $request->input('model') ?? null, ['class' => 'form-control', 'placeholder' => 'Все модели']) }}
                  </div>
                </div>
          ...
          <div class="card-footer">
            <div class="btn-group pull-right" role="group" aria-label="Basic example">
              <a class="btn btn-sm btn-danger" href="{{ route('adminlogger::index') }}" style="line-height: 24px;"><i class="fa fa-ban"></i> Reset</a>
      ...
      @if ($logs->count() > 0)
        <div class="table-responsive">
          @php $iteration = config('adminlogger.countPage') * ($logs->currentPage() - 1); @endphp
          <table class="table table-bordered">
              ...
              <tbody>
                @foreach ($logs as $log)
                  <tr>
                    <td>{{ ++$iteration }}</td>
                    <td><a href="{{ route('adminlogger::show', ['id' => $log->id]) }}">{{ !empty($log->user->fio) ? $log->user->fio : 'No name' }}</a></td>
                    <td>{{ $log->section->name_ru ?? array_get(config('adminlogger.modelsNames'), $log->model) ?? $log->model }}</td>
                    <td>{!! getBrowser($log->headers['user-agent']) !!}</td>
                    <td class="text-center">{!! array_get(config('adminlogger.events'), $log->event) !!}</td>
          ...
          <div class="d-flex justify-content-end">
            {{ $logs->links('adminlogger::admin.pagination.bootstrap-4') }}
          </div>
        </div>
      @endif
